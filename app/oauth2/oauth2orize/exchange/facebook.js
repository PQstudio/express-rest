...
module.exports = function(options, issue) {
  ...
  if (!issue) { throw new TypeError('facebookExchange exchange requires an issue callback'); }
  ...
  return function facebook_token(req, res, next) {
    ...
    if (!facebookToken) { return next(new TokenError('Missing required parameter: facebook_token', 'invalid_request')); }
    ...
    function issued(err, accessToken, refreshToken, params) {
      ...
      if (!accessToken) { return next(new TokenError('Invalid facebook access token', 'invalid_grant')); }
    ...
    try {
      ...
      if (arity == 4) {
        issue(client, facebookToken, scope, issued);
      } else { // arity == 3
        issue(client, facebookToken, issued);
      }
    } catch (ex) {
      return next(ex);
    }
